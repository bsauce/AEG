...
class FmtFinder(object):
	...
	def run(self):
		...
		if (not self.printf_plt) and self.static_flag:
			print "This binary is a static binary!"
			self.printf_plt=printf_addr
		elif (not self.printf_plt) and (not self.static_flag):
			self.printf_plt=self.get_printf_plt()

		if not self.printf_plt:
			return 0
	...
	def get_printf_plt(self):
		...
		if 'printf' in self._p.loader.main_object.plt:
			...
			return self._p.loader.main_object.plt['printf']
		return 0
...
def test1(binary):
	...
	'''
	aa=FmtFinder(binary=binary,printf_plt=0,hooks=[],read_addr=0x080486D3,write_addr=0x0804A060,write_val=0x49496565,static_flag=False)#0x080483C0
	aa.run()
	aa.result_display()
def test2(binary):
	'''
