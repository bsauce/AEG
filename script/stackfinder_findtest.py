...
class StackFinder(object):
    ...
    def run(self):
        ...
        sm = sm.explore(find=self.check,avoid=(0x0804871A,0x080486DC,0x080487CB,0x08048797,))
        if self.bug_flag:
            end_time=time.time()
    ...
    def check(self,state):
        ...
        '''
        print hex(state.se.eval(state.regs.pc,cast_to=int))
        if state.se.symbolic(state.regs.pc):
            self.bug_flag=True
            return True
        if state.regs.eip.symbolic:
            self.bug_flag=True
            return True
...
if __name__ == '__main__':
    ...
    #test2('./fmt2_test')
    #test3('./fmt_pre_test')
    test_static('/usr/downloads/test/testcase/robo_change/work/binary/vuln')
